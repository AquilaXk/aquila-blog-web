...
const WRITE_ROUTE_PATH = "/admin/posts/write"
...
export const getServerSideProps: GetServerSideProps<AdminPageProps> = async (context) => {
  ...
  if (postId) {
    return {
      redirect: {
        destination: `${WRITE_ROUTE_PATH}?postId=${encodeURIComponent(postId)}`,
        permanent: false,
      },
    }
  }
  ...
  if (source === "local-draft") {
    return {
      redirect: {
        destination: `${WRITE_ROUTE_PATH}?source=local-draft`,
        permanent: false,
      },
    }
  }
...
const AdminPostWorkspacePage: NextPage<AdminPageProps> = ({ initialMember }) => {
  ...
  const profileRole = sessionMember?.profileRole?.trim() || "역할 미설정"
  const profileBio = sessionMember?.profileBio?.trim() || "소개 문구를 아직 입력하지 않았습니다."
  const updatedText = formatDateTime(sessionMember?.modifiedAt)

  const listSummaryText = useMemo(() => {
    if (isListLoading) return "글 목록을 불러오는 중입니다."
    if (listError) return listError
    if (!listState.loadedAt) return "목록을 아직 불러오지 않았습니다."
    return `마지막 업데이트 ${formatDateTime(listState.loadedAt)} · 총 ${listState.total}건`
  }, [isListLoading, listError, listState.loadedAt, listState.total])
  ...
  const openWriteRoute = useCallback(
    async (query?: Record<string, string>) => {
      ...
      await router.push(search ? `${WRITE_ROUTE_PATH}?${search}` : WRITE_ROUTE_PATH)
    },
    [router]
  )
  ...
  return (
    <Main>
      <PageHeader>
        <HeaderCopy>
          <h1>글 작업 공간</h1>
          <p>새 글을 시작하거나, 최근 작업을 이어서 수정합니다.</p>
        </HeaderCopy>
        <SummaryRail aria-label="작업 공간 상태">
          <StatusItem>
            <span>현재 계정</span>
      ...
      <HeroSection>
        ...
        <HeroLayout>
          <HeroCopy>
            <h2>새 글 쓰기</h2>
            <p>아이디어를 바로 초안으로 시작하고, 필요하면 이전 글도 이어서 수정합니다.</p>
          </HeroCopy>
          <HeroActions>
            <PrimaryCta type="button" onClick={() => void openWriteRoute()}>
              새 글 쓰기
            </PrimaryCta>
            <SecondaryLinkButton type="button" onClick={() => continueSectionRef.current?.scrollIntoView({ behavior: "smooth", block: "start" })}>
              최근 작업 이어쓰기
            </SecondaryLinkButton>
          </HeroActions>
        </HeroLayout>
      ...
      <ResumeSection ref={continueSectionRef}>
        <SectionHeading>
          <div>
            <h2>이어서 쓰기</h2>
            <p>브라우저 임시저장과 최근 수정한 글을 먼저 보여줍니다.</p>
          </div>
        </SectionHeading>
        <ResumeGrid>
          <ResumeCard data-emphasis={localDraft ? "strong" : "soft"}>
            ...
            {localDraft ? (
              <>
                ...
                <ResumeMeta>
                  <VisibilityBadge data-tone={localDraft.visibility}>{visibilityLabelFromValue(localDraft.visibility)}</VisibilityBadge>
              ...
              <EmptyInlineState>
                ...
                <p>새 글을 쓰면 이 브라우저에 자동으로 임시저장됩니다.</p>
              </EmptyInlineState>
            )}
          </ResumeCard>
          ...
          <ResumeCard data-emphasis="soft">
            <ResumeHeader>
              ...
              <span>{isRecentLoading ? "불러오는 중" : `${recentPosts.length}개`}</span>
            </ResumeHeader>
            {renderRecentEdited()}
          </ResumeCard>
        </ResumeGrid>
        {!hasAnyResumeTarget && (
          <WorkspaceEmpty>
            ...
            <p>바로 새 글을 시작하면 다음부터는 여기서 이어서 작업할 수 있습니다.</p>
      ...
      <ListSection ref={listSectionRef}>
        <SectionHeading>
          <div>
            <h2>글 목록</h2>
            <p>오래된 글을 찾거나, 상태를 확인하고, 필요하면 바로 수정합니다.</p>
          </div>
          <ListMeta>
        ...
        <FilterRail>
          ...
          <SearchField>
            ...
            <input
              ...
              placeholder={listScope === "active" ? "제목이나 본문을 찾습니다" : "삭제된 글을 찾습니다"}
              value={listKw}
              onChange={(event) => {
      ...
      <UserContextCard>
        <ProfileFrame>
          {profileSrc ? <ProfileImage src={profileSrc} alt={displayName} fillContainer priority /> : <ProfileFallback>{displayNameInitial}</ProfileFallback>}
        </ProfileFrame>
        <ProfileCopy>
          <strong>{displayName}</strong>
        ...
        <Link href="/admin/profile" passHref legacyBehavior>
          <ProfileLink>프로필 정리</ProfileLink>
        </Link>
      </UserContextCard>
    </Main>
  )
...
const UserContextCard = styled.section`
  display: grid;
