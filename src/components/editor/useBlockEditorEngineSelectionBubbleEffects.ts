...
import { TABLE_DRAG_SELECTION_TEXT_ATTR, TABLE_DRAG_SELECTION_TEXT_SELECTOR, cancelActiveTableCellScrollPreserves, cancelActiveTableCellTextSelectionPreserves, clearTableTextSelectionForStructuralSelection, collapseStaleTableEditorSelection, getTableTextSelectionClearGeneration, isTableStructuralSelectionOwnerActive, isTableTextSelectionClearGenerationCurrent, preservePendingSingleCellNativeTextSelectionAcrossFrames, preserveTableCellTextSelectionAcrossFrames, resolveTableTextCellAtPoint, resolveTableTextDragScrollAnchorFromEvent, resolveTableTextSelectionRangeCells, restoreTableCellTextSelectionIfEscaped, selectTableCellTextRange, suppressSingleCellNativeTextSelectionPreserve, watchTableCellTextSelectionExternalClear } from "./tableTextSelectionModel"
import { areFloatingBubbleStatesEqual, hasNativeEditorTextSelection, hideFloatingBubbleState, resolveFloatingBubbleStateFromCoords, resolveHeadingSelectionBubbleState, resolveOcclusionAwareTextBubbleState, type FloatingBubbleState } from "./useFloatingBubbleState"
...
const shouldCancelTableTextScrollPreserve = (scrollAnchor: WindowScrollAnchor) => () => {
  ...
  return (
    Math.abs(window.scrollX - scrollAnchor.x) > TABLE_TEXT_SCROLL_PRESERVE_CANCEL_DISTANCE_PX ||
    Math.abs(currentY - scrollAnchor.y) > TABLE_TEXT_SCROLL_PRESERVE_CANCEL_DISTANCE_PX
  )
}
...
export const useBlockEditorEngineSelectionBubbleEffects = ({
  ...
  useEffect(() => {
    ...
    const rememberTableTextDragStart = (event: MouseEvent | PointerEvent, _allowTableControlCellFallback = false, allowOutsideCoveredControlFallback = false) => { if (isTableStructuralSelectionOwnerActive()) return null
      ...
      const scrollAnchor = tableTextCell instanceof HTMLElement ? resolveTableTextDragScrollAnchorFromEvent(event, tableTextCell) : { x: window.scrollX, y: document.scrollingElement?.scrollTop ?? window.scrollY }
    ...
    const syncBubble = () => {
      ...
      const headingBubbleState = resolveHeadingSelectionBubbleState(baseBubbleState, activeEditor.view.dom)
      const nextBubbleState = resolveOcclusionAwareTextBubbleState(headingBubbleState, activeEditor.view.dom)
      setBubbleState((prev) =>
    ...
    const completeTextDragSelection = (event: PointerEvent | MouseEvent) => {
      ...
      if (activeEditor && tableTextDragStart && (tableTextDragMoved || tableTextDragStart.coveredControlFallback)) { const completedTableTextDrag = tableTextDragStart, completedMultiCellDrag = Boolean(completedTableTextDrag.endCell && completedTableTextDrag.endCell !== completedTableTextDrag.cell); cancelTableTextDragPreserves()
        if (completedMultiCellDrag && completedTableTextDrag.endCell) { const endCell = completedTableTextDrag.endCell; selectTableCellTextRange(completedTableTextDrag.cell, endCell); cleanupTableDragSelectionPreserve = preserveTableCellTextSelectionAcrossFrames(activeEditor, completedTableTextDrag.cell, completedTableTextDrag.scrollAnchor, () => endCell); event.preventDefault(); event.stopPropagation(); syncBubbleOnMouseUpRef.current = true }
        else { let delayedSingleCellRestoreCancelled = false; const delayedSingleCellRestoreCancelEvents = ["wheel", "pointerdown", "mousedown", "keydown"] as const, shouldCancelDelayedSingleCellRestore = shouldCancelTableTextScrollPreserve(completedTableTextDrag.scrollAnchor), clearDelayedSingleCellRestoreCancelListeners = () => delayedSingleCellRestoreCancelEvents.forEach((eventType) => window.removeEventListener(eventType, cancelDelayedSingleCellRestore, true)), cancelDelayedSingleCellRestore = () => { delayedSingleCellRestoreCancelled = true; clearDelayedSingleCellRestoreCancelListeners(); cancelActiveTableWindowScrollPreserve(); cancelActiveTableCellScrollPreserves(); suppressSingleCellNativeTextSelectionPreserve() }, expireDelayedSingleCellRestore = () => { delayedSingleCellRestoreCancelled = true; clearDelayedSingleCellRestoreCancelListeners() }, restoreSingleCellAfterRelease = () => delayedSingleCellRestoreCancelled || shouldCancelDelayedSingleCellRestore() ? false : preservePendingSingleCellNativeTextSelectionAcrossFrames(completedTableTextDrag.cell, completedTableTextDrag.scrollAnchor)
          preserveWindowScrollPositionAcrossFrames(completedTableTextDrag.scrollAnchor, 600, 4, 10_000, false, false, true, false, false, shouldCancelDelayedSingleCellRestore, true, Number.POSITIVE_INFINITY, "table")
          const restored = restoreActiveTableTextDragSelection(true, true, false), preservedSingleCell = preservePendingSingleCellNativeTextSelectionAcrossFrames(completedTableTextDrag.cell, completedTableTextDrag.scrollAnchor)
          delayedSingleCellRestoreCancelEvents.forEach((eventType) => window.addEventListener(eventType, cancelDelayedSingleCellRestore, { capture: true, passive: eventType !== "keydown", once: true })); window.requestAnimationFrame(restoreSingleCellAfterRelease); window.setTimeout(restoreSingleCellAfterRelease, 80); window.setTimeout(restoreSingleCellAfterRelease, 240); window.setTimeout(expireDelayedSingleCellRestore, 360)
          if (restored || preservedSingleCell) { event.preventDefault(); event.stopPropagation(); syncBubbleOnMouseUpRef.current = true; if (!preservedSingleCell) window.requestAnimationFrame(() => restoreTableCellTextSelectionIfEscaped(activeEditor, completedTableTextDrag.cell, null, true, true, false, completedTableTextDrag.endCell)) } }
      }
