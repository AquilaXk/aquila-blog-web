...
import { TABLE_DRAG_SELECTION_TEXT_ATTR, TABLE_DRAG_SELECTION_TEXT_SELECTOR, cancelActiveTableCellTextSelectionPreserves, clearTableTextSelectionForStructuralSelection, collapseStaleTableEditorSelection, getTableTextSelectionClearGeneration, isTableStructuralSelectionOwnerActive, isTableTextSelectionClearGenerationCurrent, preserveTableCellTextSelectionAcrossFrames, resolveTableTextCellAtPoint, resolveTableTextSelectionRangeCells, restoreTableCellTextSelectionIfEscaped, selectTableCellTextRange, watchTableCellTextSelectionExternalClear } from "./tableTextSelectionModel"
...
export const useBlockEditorEngineSelectionBubbleEffects = ({
  ...
  useEffect(() => {
    ...
    const handleEditorMouseDownCapture = (event: MouseEvent) => {
      ...
      if (codeShellTarget) {
        const codeTextSurfaceTarget = findCodeTextSurfaceTarget(targetElement, pointElements)
