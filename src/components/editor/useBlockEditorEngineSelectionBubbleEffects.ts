...
import { TABLE_DRAG_SELECTION_TEXT_ATTR, TABLE_DRAG_SELECTION_TEXT_SELECTOR, cancelActiveTableCellTextSelectionPreserves, clearTableTextSelectionForStructuralSelection, collapseStaleTableEditorSelection, getTableTextSelectionClearGeneration, isTableTextSelectionClearGenerationCurrent, preserveTableCellTextSelectionAcrossFrames, resolveTableTextCellAtPoint, resolveTableTextSelectionRangeCells, restoreTableCellTextSelectionIfEscaped, selectTableCellTextRange, watchTableCellTextSelectionExternalClear } from "./tableTextSelectionModel"
...
export const useBlockEditorEngineSelectionBubbleEffects = ({
  ...
  useEffect(() => {
    ...
    const completeTextDragSelection = (event: PointerEvent | MouseEvent) => {
      ...
      if (activeEditor && hasTableStructuralSelection(activeEditor) && window.getSelection()?.toString().trim()) { tableTextDragStartRef.current = null; tableTextDragPendingStartRef.current = null; nonTableTextDragStartRef.current = null; mouseTextSelectionInProgressRef.current = false; cancelTableTextDragPreserves(); cancelActiveTableCellTextSelectionPreserves(); return } if (!mouseTextSelectionInProgressRef.current && !tableTextDragStart && !pendingTableTextDragStart && !codeTextDragStart && !nativeTableRangeSnapshot) return
