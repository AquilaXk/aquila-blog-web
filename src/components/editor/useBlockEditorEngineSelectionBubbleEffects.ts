...
import { TABLE_DRAG_SELECTION_TEXT_ATTR, TABLE_DRAG_SELECTION_TEXT_SELECTOR, cancelActiveTableCellTextSelectionPreserves, collapseStaleTableEditorSelection, getTableTextSelectionClearGeneration, isTableTextSelectionClearGenerationCurrent, preserveTableCellTextSelectionAcrossFrames, resolveTableTextCellAtPoint, resolveTableTextSelectionRangeCells, restoreTableCellTextSelectionIfEscaped, selectTableCellTextRange, watchTableCellTextSelectionExternalClear } from "./tableTextSelectionModel"
...
const CODE_BLOCK_EDITOR_CONTENT_SELECTOR = ".aq-code-editor-content", CODE_BLOCK_INTERACTIVE_CONTROL_SELECTOR = "[data-code-language-control='true'], [data-block-handle-rail='true'], [data-block-menu-root='true']", BLOCK_EDITOR_ROOT_SELECTOR = "[data-testid='block-editor-prosemirror'], .ProseMirror"
...
export const useBlockEditorEngineSelectionBubbleEffects = ({
  ...
  useEffect(() => {
    ...
    const completeTextDragSelection = (event: PointerEvent | MouseEvent) => {
      ...
      if (activeEditor && nativeTableRangeSnapshot) { const targetElement = event.target instanceof Element ? event.target : event.target instanceof Node ? event.target.parentElement : null, releasedOnTableControl = Boolean(targetElement?.closest(TABLE_TEXT_DRAG_CONTROL_SELECTOR)), releasedOnAxisRail = Boolean(targetElement?.closest("[data-table-axis-rail='true']")), clearGeneration = getTableTextSelectionClearGeneration(), restoreNativeTableRangeSnapshot = () => { if (isTableTextSelectionClearGenerationCurrent(clearGeneration)) restoreTableCellTextSelectionIfEscaped(activeEditor, nativeTableRangeSnapshot.anchorCell, null, true, true, false, nativeTableRangeSnapshot.pointCell) }; if (!releasedOnTableControl || releasedOnAxisRail) restoreNativeTableRangeSnapshot(); window.requestAnimationFrame(restoreNativeTableRangeSnapshot); window.setTimeout(() => { if (!isTableTextSelectionClearGenerationCurrent(clearGeneration)) return; const text = selectTableCellTextRange(nativeTableRangeSnapshot.anchorCell, nativeTableRangeSnapshot.pointCell); nativeTableRangeSnapshot.anchorCell.setAttribute("data-table-drag-selection-text", text || nativeTableRangeSnapshot.anchorCell.textContent?.replace(/\s+/g, " ").trim() || "") }, 80) }
      updateTableTextDragEndCellFromPoint(event)
