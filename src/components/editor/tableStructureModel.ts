...
export const hasTableContextInResolvedPos = (resolvedPos: { depth: number; node: (depth: number) => { type: { name: string } } }) => {
...
export const getActiveTableStructureState = (editor?: TiptapEditor | null): ActiveTableStructureState => {
  if (!editor) {
    return {
      hasHeaderRow: false,
...
export const canShrinkTableAxisAtEnd = (tableNode: ProseMirrorNode, axis: TableAxis): boolean => {
...
export const countShrinkableTableAxisAtEnd = (tableNode: ProseMirrorNode, axis: TableAxis): number => {
  const rowCount = tableNode.childCount
