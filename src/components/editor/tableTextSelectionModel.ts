...
const preserveNativeSingleCellRangeAcrossFrames = (cell: HTMLElement, range: Range, scrollAnchorOverride?: WindowScrollAnchor | null) => {
  ...
  function armPointerCancel() { pointerCancelArmed = true } function restoreScrollPosition() { const currentY = document.scrollingElement?.scrollTop ?? window.scrollY; if (Math.abs(window.scrollX - scrollAnchor.x) > 4 || Math.abs(currentY - scrollAnchor.y) > 4) window.scrollTo(scrollAnchor.x, scrollAnchor.y) } function restoreOnScroll() { if (!cancelled && isTableTextSelectionClearGenerationCurrent(clearGeneration) && cell.isConnected) restoreScrollPosition() }
