...
export const isTableTextSelectionClearGenerationCurrent = (generation: number) => generation === tableTextSelectionClearGeneration
const getNow = () => (typeof performance !== "undefined" ? performance.now() : Date.now())
const isTableTextSelectionFinalizeSuppressed = () => getNow() < tableTextSelectionFinalizeSuppressedUntil
const shouldCancelTableTextScrollPreserve = (scrollAnchor: WindowScrollAnchor) => () => Math.abs(window.scrollX - scrollAnchor.x) > TABLE_TEXT_SCROLL_PRESERVE_CANCEL_DISTANCE_PX || Math.abs((document.scrollingElement?.scrollTop ?? window.scrollY) - scrollAnchor.y) > TABLE_TEXT_SCROLL_PRESERVE_CANCEL_DISTANCE_PX
export const suppressSingleCellNativeTextSelectionPreserve = (durationMs = 1_200) => { suppressSingleCellNativeTextSelectionUntil = Math.max(suppressSingleCellNativeTextSelectionUntil, getNow() + durationMs); activeSingleCellNativeTextSelectionCancel?.(); pendingSingleCellNativeTextSelection = null; lastObservedSingleCellNativeTextSelection = null; lastSingleCellTextDragStart = null }
export const markTableStructuralSelectionOwner = (durationMs = 720) => { tableStructuralSelectionOwnerUntil = Math.max(tableStructuralSelectionOwnerUntil, getNow() + durationMs) }
export const isTableStructuralSelectionOwnerActive = () => getNow() < tableStructuralSelectionOwnerUntil; export const clearTableStructuralSelectionOwner = () => { tableStructuralSelectionOwnerUntil = 0 }
...
const clearPendingTableTextSelectionState = () => { pendingTableTextSelectionRangeCells = null; explicitTableTextDragStart = null; pendingSingleCellNativeTextSelection = null; lastObservedSingleCellNativeTextSelection = null; lastSingleCellTextDragStart = null }
...
const resolveSingleCellNativeScrollAnchor = (cell: HTMLElement, scrollAnchorOverride?: WindowScrollAnchor | null) => { const cellText = normalizeCellText(cell), now = getNow(), currentScrollAnchor = { x: window.scrollX, y: document.scrollingElement?.scrollTop ?? window.scrollY }, isSameIdentityCell = (tracked: { cell: HTMLElement; expiresAt: number; identity: SingleCellTableCellIdentity; text: string } | null) => { if (!tracked || tracked.expiresAt <= now) return false; const trackedCell = tracked.cell === cell ? cell : resolveSingleCellByIdentity(tracked.identity); return Boolean(trackedCell === cell || (trackedCell?.isConnected && trackedCell.closest("table") === cell.closest("table") && (tracked.text === cellText || Boolean(tracked.text && cellText.includes(tracked.text))))) }, matchesLastStart = isSameIdentityCell(lastSingleCellTextDragStart), matchesHover = isSameIdentityCell(lastTableTextPointerHover); return [scrollAnchorOverride ?? null, explicitTableTextDragStart?.cell === cell ? explicitTableTextDragStart.scrollAnchor : null, matchesLastStart ? lastSingleCellTextDragStart!.scrollAnchor : null, lastObservedSingleCellNativeTextSelection?.cell === cell && lastObservedSingleCellNativeTextSelection.expiresAt > now ? lastObservedSingleCellNativeTextSelection.scrollAnchor : null, matchesHover ? lastTableTextPointerHover!.scrollAnchor : null].reduce<WindowScrollAnchor | null>((best, candidate) => !candidate ? best : !best || (Math.abs(best.x - candidate.x) <= 4 && candidate.y > best.y + 24) ? candidate : best, null) ?? currentScrollAnchor }
const preserveNativeSingleCellRangeAcrossFrames = (cell: HTMLElement, range: Range, scrollAnchorOverride?: WindowScrollAnchor | null) => {
  if (getNow() < suppressSingleCellNativeTextSelectionUntil || (!range.toString().trim() && !normalizeCellText(cell))) return false; activeSingleCellNativeTextSelectionCancel?.()
  rememberSingleCellNativeTextSelectionSnapshot(cell, range, scrollAnchorOverride)
  const clearGeneration = tableTextSelectionClearGeneration, scrollAnchor = resolveSingleCellNativeScrollAnchor(cell, scrollAnchorOverride), startedAt = getNow(); let cancelled = false, frame = 0, pointerCancelArmed = !explicitTableTextDragStart
  const shouldCancelForFarScroll = shouldCancelTableTextScrollPreserve(scrollAnchor)
  function armPointerCancel() { pointerCancelArmed = true } function restoreScrollPosition() { const currentY = document.scrollingElement?.scrollTop ?? window.scrollY; if (shouldCancelForFarScroll()) { cancel(); return }; if (Math.abs(window.scrollX - scrollAnchor.x) > 4 || Math.abs(currentY - scrollAnchor.y) > 4) window.scrollTo(scrollAnchor.x, scrollAnchor.y) } function restoreOnScroll() { if (!cancelled && isTableTextSelectionClearGenerationCurrent(clearGeneration) && cell.isConnected) restoreScrollPosition() }
  function cleanup() { window.removeEventListener("pointerdown", cancel, true); window.removeEventListener("mousedown", cancel, true); window.removeEventListener("pointerup", armPointerCancel, true); window.removeEventListener("mouseup", armPointerCancel, true); window.removeEventListener("wheel", cancel, true); window.removeEventListener("scroll", restoreOnScroll, true); window.removeEventListener("keydown", cancel, true); if (activeSingleCellNativeTextSelectionCancel === cancel) activeSingleCellNativeTextSelectionCancel = null }
  ...
  const restore = () => { if (cancelled) return; if (!isTableTextSelectionClearGenerationCurrent(clearGeneration) || !cell.isConnected) { cleanup(); return }; if (shouldCancelForFarScroll()) { cancel(); return }; try { const selection = window.getSelection(), anchorElement = resolveElement(selection?.anchorNode), focusElement = resolveElement(selection?.focusNode), hasOwnedSelection = Boolean(selection?.rangeCount && selection.toString().trim() && anchorElement && focusElement && cell.contains(anchorElement) && cell.contains(focusElement)); if (!hasOwnedSelection) { selection?.removeAllRanges(); selection?.addRange(range.cloneRange()) }; restoreScrollPosition() } catch { cleanup(); return }; frame += 1; if (frame < SINGLE_CELL_NATIVE_SELECTION_PRESERVE_FRAMES || getNow() - startedAt < SINGLE_CELL_NATIVE_SELECTION_PRESERVE_MIN_MS) window.requestAnimationFrame(restore); else cleanup() }
  activeSingleCellNativeTextSelectionCancel = cancel; window.addEventListener("pointerdown", cancel, true); window.addEventListener("mousedown", cancel, true); window.addEventListener("pointerup", armPointerCancel, { capture: true, once: true }); window.addEventListener("mouseup", armPointerCancel, { capture: true, once: true }); window.addEventListener("wheel", cancel, { capture: true, passive: true, once: true }); window.addEventListener("scroll", restoreOnScroll, { capture: true, passive: true }); window.addEventListener("keydown", cancel, { capture: true, once: true }); restore(); return true
}
const rememberSingleCellNativeTextSelectionSnapshot = (cell: HTMLElement, range: Range, scrollAnchorOverride?: WindowScrollAnchor | null) => { const text = (range.toString() || normalizeCellText(cell)).replace(/\s+/g, " ").trim(); if (!text || getNow() < suppressSingleCellNativeTextSelectionUntil) return; const identity = createSingleCellTableIdentity(cell), scrollAnchor = resolveSingleCellNativeScrollAnchor(cell, scrollAnchorOverride); pendingSingleCellNativeTextSelection = { cell, identity, range: range.cloneRange() }; lastObservedSingleCellNativeTextSelection = { cell, expiresAt: getNow() + SINGLE_CELL_NATIVE_SELECTION_PRESERVE_MIN_MS, identity, range: range.cloneRange(), scrollAnchor, text } }
...
const restorePendingSingleCellNativeTextSelection = (cell: HTMLElement | null | undefined, scrollAnchor?: WindowScrollAnchor | null) => { if (!cell || (!pendingSingleCellNativeTextSelection && !lastObservedSingleCellNativeTextSelection)) return false; const snapshot = resolveSingleCellNativeTextSelectionSnapshot(cell); return snapshot ? preserveNativeSingleCellRangeAcrossFrames(snapshot.cell, snapshot.range, scrollAnchor) : false }
...
export const finalizeTableTextSelectionFromPoint = (clientX: number, clientY: number, target?: EventTarget | Node | null) => {
  ...
  explicitTableTextDragStart = null
  if (isTableStructuralSelectionOwnerActive() || (targetElement?.closest(TABLE_TEXT_SELECTION_CONTROL_SELECTOR) && !explicitDragStart)) { clearPendingTableTextSelectionState(); cancelActiveTableCellTextSelectionPreserves(); clearTableTextRangeHighlight({ markBlur: false }); window.getSelection()?.removeAllRanges(); return false }
  ...
  const sameCellNativeSelectionPreserved = explicitDragStart ? (Math.abs(clientX - explicitDragStart.x) > 4 || Math.abs(clientY - explicitDragStart.y) > 4) && (preserveSingleCellNativeTextSelection(explicitDragStart.cell, explicitDragStart.scrollAnchor) || restorePendingSingleCellNativeTextSelection(explicitDragStart.cell, explicitDragStart.scrollAnchor) || preservePendingSingleCellNativeTextSelectionAcrossFrames(explicitDragStart.cell, explicitDragStart.scrollAnchor)) : preserveSingleCellNativeTextSelection(escapedSelectionCell instanceof HTMLElement ? escapedSelectionCell : null)
...
const resolveTableDragStartScrollAnchor = (currentScrollAnchor: WindowScrollAnchor, hoverAnchor: WindowScrollAnchor | null) => !hoverAnchor || Math.abs(currentScrollAnchor.x - hoverAnchor.x) > 4 ? currentScrollAnchor : currentScrollAnchor.y > hoverAnchor.y + 24 ? currentScrollAnchor : hoverAnchor
export const resolveTableTextDragScrollAnchorFromEvent = (event: MouseEvent | PointerEvent, startCell?: HTMLElement | null) => { const directStartCell = resolveTableTextCellAtPoint(event.clientX, event.clientY, event.target), currentScrollAnchor = { x: window.scrollX, y: document.scrollingElement?.scrollTop ?? window.scrollY }, hoverCell = lastTableTextPointerHover && lastTableTextPointerHover.expiresAt > getNow() && lastTableTextPointerHover.cell.isConnected && Math.abs(event.clientX - lastTableTextPointerHover.x) <= 32 && Math.abs(event.clientY - lastTableTextPointerHover.y) <= 32 ? lastTableTextPointerHover : null, resolvedStartCell = startCell ?? (directStartCell instanceof HTMLElement ? directStartCell : hoverCell?.cell ?? null), startCellText = normalizeCellText(resolvedStartCell), hoverAnchor = hoverCell && resolvedStartCell instanceof HTMLElement && (hoverCell.cell === resolvedStartCell || hoverCell.text === startCellText || Boolean(hoverCell.text && startCellText.includes(hoverCell.text))) ? hoverCell.scrollAnchor : null; return resolveTableDragStartScrollAnchor(currentScrollAnchor, hoverAnchor) }
const rememberExplicitTableTextDragStart = (event: MouseEvent | PointerEvent) => { if (event.button !== 0 || ("pointerType" in event && event.pointerType && event.pointerType !== "mouse")) return; const targetElement = resolveElement(event.target); if (isTableStructuralSelectionOwnerActive() || targetElement?.closest(TABLE_TEXT_SELECTION_CONTROL_SELECTOR)) { clearPendingTableTextSelectionState(); return }; if (event.type === "mousedown" && explicitTableTextDragStart && Math.abs(event.clientX - explicitTableTextDragStart.x) <= 4 && Math.abs(event.clientY - explicitTableTextDragStart.y) <= 4) return; pendingSingleCellNativeTextSelection = null; lastObservedSingleCellNativeTextSelection = null; const hoverCell = lastTableTextPointerHover && lastTableTextPointerHover.expiresAt > getNow() && lastTableTextPointerHover.cell.isConnected && Math.abs(event.clientX - lastTableTextPointerHover.x) <= 32 && Math.abs(event.clientY - lastTableTextPointerHover.y) <= 32 ? lastTableTextPointerHover : null, directStartCell = resolveTableTextCellAtPoint(event.clientX, event.clientY, event.target), startCell = directStartCell instanceof HTMLElement ? directStartCell : hoverCell?.cell ?? null, startCellText = normalizeCellText(startCell), scrollAnchor = resolveTableTextDragScrollAnchorFromEvent(event, startCell instanceof HTMLElement ? startCell : null); explicitTableTextDragStart = startCell instanceof HTMLElement ? { cell: startCell, scrollAnchor, x: event.clientX, y: event.clientY } : null; if (explicitTableTextDragStart) { cancelAllWindowScrollPreserves(); preserveWindowScrollPositionAcrossFrames(scrollAnchor, 420, 4, 7_000, false, false, true, false, false, shouldCancelTableTextScrollPreserve(scrollAnchor), true, Number.POSITIVE_INFINITY, "table") }; lastSingleCellTextDragStart = explicitTableTextDragStart && startCell instanceof HTMLElement ? { ...explicitTableTextDragStart, expiresAt: getNow() + SINGLE_CELL_NATIVE_SELECTION_PRESERVE_MIN_MS, identity: createSingleCellTableIdentity(startCell), text: startCellText } : null }
...
export const collapseStaleTableEditorSelection = (editor: TiptapEditor, pointer?: { clientX: number; clientY: number; target?: EventTarget | Node | null }) => {
  if (!isEditorSelectionInsideTable(editor)) return false
  const { doc, selection } = editor.state, targetElement = resolveElement(pointer?.target), pointerTableTarget = Boolean(targetElement?.closest("th, td, table, .tableWrapper, .aq-table-shell")), pointerPos = pointer && !pointerTableTarget ? editor.view.posAtCoords({ left: pointer.clientX, top: pointer.clientY })?.pos ?? null : null
  if (pointer && !pointerTableTarget && (pendingSingleCellNativeTextSelection || lastObservedSingleCellNativeTextSelection || isWindowSelectionInsideEditorTable(editor.view.dom))) suppressSingleCellNativeTextSelectionPreserve()
  const collapsePos = Math.max(0, Math.min(doc.content.size, pointerPos ?? selection.to))
  try {
    const nextSelection = TextSelection.near(doc.resolve(collapsePos), pointerPos === null ? -1 : 1)
...
export const restoreTableCellTextSelectionIfEscaped = (
  ...
  if (scrollAnchor) {
    const cancelScrollPreserve = preserveWindowScrollPositionAcrossFrames(
      ...
      true,
      false,
      () => shouldCancelTableTextScrollPreserve(scrollAnchor)() || !hasOwnedTableCellTextSelection(editor, startedCell)
    )
...
export const preserveTableCellTextSelectionAcrossFrames = (
  ...
  const clearGeneration = tableTextSelectionClearGeneration
  let frame = 0
  let cancelled = false, restoring = false, lastUserScrollIntentAt = 0
  const cancelScrollPreserve = preserveWindowScrollPositionAcrossFrames(
    scrollAnchor,
    TABLE_TEXT_DRAG_SCROLL_PRESERVE_FRAMES,
    4,
  ...
  const cleanup = () => {
    ...
    window.removeEventListener("touchstart", rememberUserScrollIntent, true); window.removeEventListener("touchmove", rememberUserScrollIntent, true); window.removeEventListener("scroll", cancelIfRecentUserScrollIntent, true)
  ...
  const rememberUserScrollIntent = () => { lastUserScrollIntentAt = getNow() }
  const cancelIfRecentUserScrollIntent = () => { if (lastUserScrollIntentAt && getNow() - lastUserScrollIntentAt <= 350) cancel() }
  ...
  window.addEventListener("keydown", cancel, { capture: true, once: true })
  window.addEventListener("touchstart", rememberUserScrollIntent, { capture: true, passive: true }); window.addEventListener("touchmove", rememberUserScrollIntent, { capture: true, passive: true }); window.addEventListener("scroll", cancelIfRecentUserScrollIntent, { capture: true, passive: true })
