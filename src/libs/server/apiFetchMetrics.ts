...
import { classifyBackendRoute, getRuntimeMetrics, type BackendFetchResult, type BackendFetchSource } from "src/libs/server/runtimeMetrics"
...
export const registerServerApiFetchMetrics = () => {
  ...
  registerServerApiFetchMetricsFactory((path, headers) => {
    ...
    const source: BackendFetchSource = activeContext?.source ?? "isr"
    ...
    return {
      requestId,
      observe: (result: BackendFetchResult) => {
