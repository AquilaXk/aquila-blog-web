...
test("settings account page deletes account after password reauthentication", async ({ page }) => {
  ...
  await expect(page.getByRole("heading", { name: "계정 보안" })).toBeVisible()
  const retentionNotice = page.getByText(/법적 의무와 보안 감사에 필요한 최소 기록/)
  const passwordInput = page.getByLabel("비밀번호 재확인 (이메일 계정)")
  await expect(retentionNotice).not.toHaveClass(/jsx-undefined/)
  await expect(retentionNotice).toHaveCSS("margin-bottom", "18px")
  await expect(page.locator("form.deleteForm")).toHaveCSS("display", "grid")
  await expect(passwordInput).toHaveCSS("padding-top", "11px")
  await expect(passwordInput).toHaveCSS("border-top-width", "1px")
  await expect(retentionNotice).toContainText("최소 기록은")
  ...
  await expect(page.getByRole("region", { name: "계정 탈퇴 완료" })).toHaveCSS("margin-top", "48px")
