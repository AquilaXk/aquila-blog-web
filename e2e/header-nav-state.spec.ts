...
test("헤더 auth shell은 pre-paint bootstrap과 display-gated 슬롯을 유지한다", () => {
  ...
  expect(navBarSource).toContain("useLayoutEffect as useReactLayoutEffect")
  expect(navBarSource).toContain('typeof window === "undefined" ? useEffect : useReactLayoutEffect')
  expect(navBarSource).toContain("useIsomorphicLayoutEffect(() => {")
  expect(navBarSource).not.toContain("useLayoutEffect(() => {")
