...
test.describe("mermaid editor view", () => {
  test("머메이드 블록은 코드/코드+Mermaid/Mermaid 보기 모드를 제공한다", () => {
    ...
    expect(source).toContain("data-selection-active={isCodeSelectionActive}")
    expect(source).toContain("const [isCodeSelectionActive, setIsCodeSelectionActive] = useState(false)")
    expect(source).toContain("onSelect={syncCodeSelectionState}")
  })
})
