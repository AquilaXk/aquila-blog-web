...
test.describe("mermaid editor view", () => {
  test("머메이드 블록은 코드/코드+Mermaid/Mermaid 보기 모드를 제공한다", () => {
    ...
    expect(source).toContain("const showCodePane = viewMode !== \"preview\"")
    expect(source).toContain("const showPreviewPane = viewMode !== \"code\"")
    expect(source).toContain("lazyViewport: false")
    ...
    expect(source).toContain("color: transparent !important;")
    expect(source).toContain("-webkit-text-fill-color: transparent !important;")
    expect(source).toContain("&::selection {")
    expect(source).toContain("background: rgba(59, 130, 246, 0.28);")
  })
})
