...
test.describe("admin profile state contract", () => {
  test("profile 작업공간은 공통 section nav/action dock primitive 위에서 반응형 분기를 유지한다", () => {
    ...
    expect(source).toContain("const PreviewStatusRail = styled(AdminInfoStatusList)`")
    expect(source).toContain("<AdminInfoStatusItem key={`preview-${item.label}`} data-tone={item.tone}>")
