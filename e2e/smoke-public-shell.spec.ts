...
test.describe("core smoke public shell", () => {
  ...
  expect(headerSource).toContain("@media (max-width: ${layoutBreakpoint.navCompact}px)")
  ...
  expect(navBarSource).toContain("@media (max-width: ${layoutBreakpoint.navCompact}px)")
  ...
  expect(navBarSource).toContain('createPortal(')
  expect(navBarSource).toContain("document.body")
  expect(navBarSource).toContain('from "src/design-system/useModalFocusTrap"')
  ...
  expect(navBarSource).toContain(".adminLink,\n    .logoutBtn {\n      display: none;")
  expect(navBarSource).toContain(".primaryLinks a {\n      min-height: ${control.lg}px;")
