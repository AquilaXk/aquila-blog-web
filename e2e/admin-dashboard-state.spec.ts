...
test.describe("admin dashboard state contract", () => {
  ...
  test("운영 대시보드는 주기·포커스 갱신과 수동 새로고침·수집 실패 표면화를 제공한다", () => {
    ...
    expect(source).toContain("healthCollectionFailed = systemHealthQuery.isError")
    expect(source).toContain("snapshotCollectionFailed = dashboardSnapshotQuery.isError")
