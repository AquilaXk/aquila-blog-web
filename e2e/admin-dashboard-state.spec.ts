...
test.describe("admin dashboard state contract", () => {
  ...
  test("운영 대시보드는 주기·포커스 갱신과 수동 새로고침·수집 실패 표면화를 제공한다", () => {
    const source = readDashboardRouteSource("AdminDashboardWorkspacePage.tsx")
    const viewSource = readDashboardRouteSource("AdminDashboardWorkspaceView.tsx")
    const sectionsSource = readDashboardRouteSource("AdminDashboardWorkspaceSections.tsx")
    const modelSource = readFileSync(path.resolve(__dirname, "../src/routes/Admin/AdminDashboardWorkspaceModel.ts"), "utf8")
    ...
    expect(source).toContain("systemHealthQuery.refetch()")
    expect(source).toContain("dashboardSnapshotQuery.refetch()")
    expect(source).toContain("isManualRefreshing")
    expect(source).toContain("setIsManualRefreshing(true)")
    expect(source).toContain(".finally(() => {")
    expect(source).not.toContain("isRefreshing = systemHealthQuery.isFetching")
    expect(source).not.toContain("dashboardSnapshotQuery.isFetching")
    expect(source).toContain("formatDashboardFreshnessLabel")
    expect(source).toContain("resolveDashboardDataUpdatedAt")
    expect(source).toContain("healthCollectionFailed = isDashboardQueryCollectionFailed(systemHealthQuery)")
    expect(source).toContain("snapshotCollectionFailed = isDashboardQueryCollectionFailed(dashboardSnapshotQuery)")
    expect(source).toContain("collectionFailed = healthCollectionFailed || snapshotCollectionFailed")
    expect(source).toContain("snapshotCollectionFailed={snapshotCollectionFailed}")
    expect(modelSource).toContain('export const DASHBOARD_COLLECTION_FAILED_LABEL = "수집 실패 · 재시도"')
    expect(modelSource).toContain("export const formatDashboardFreshnessLabel")
    expect(modelSource).toContain("export const resolveDashboardDataUpdatedAt")
    expect(modelSource).toContain("export const isDashboardQueryCollectionFailed")
    expect(modelSource).toContain("export const resolveDashboardCollectionLabel")
    expect(sectionsSource).toContain("DashboardRefreshControls")
    expect(sectionsSource).toContain("DASHBOARD_REFRESH_LABEL")
    expect(viewSource).toContain("DashboardRefreshControls")
    expect(viewSource).toContain("freshnessLabel")
    expect(viewSource).toContain("snapshotCollectionFailed")
    expect(viewSource).toContain("수집 실패 · 재시도")
  })
})
