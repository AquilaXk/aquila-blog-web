...
test("관리자 글 목록 surface는 심각도 높은 접근성 위반이 없다", async ({ page }, testInfo) => {
  ...
  const activeScopeButton = page.getByRole("button", { name: "활성 글" })
  const deletedScopeButton = page.getByRole("button", { name: "삭제 글" })
  await expect(activeScopeButton).toHaveAttribute("aria-pressed", "true")
  await expect(deletedScopeButton).toHaveAttribute("aria-pressed", "false")
  await deletedScopeButton.click()
  await expect(activeScopeButton).toHaveAttribute("aria-pressed", "false")
...
test("법적 정책 route는 200% zoom과 dark mode에서 심각도 높은 접근성 위반이 없다", async ({
  page,
}, testInfo) => {
  await setSchemeCookie(page, "dark")
  ...
  await expect(page.getByText(/contentSha256/)).toBeVisible()
  await page.addStyleTag({ content: "html { zoom: 2; }" })
  await expectNoHorizontalOverflow(page)
  await expectPrimaryLandmarks(page)
  await expectLaunchGateAccessibility(page, testInfo, "legal-privacy-dark-zoom")
})
