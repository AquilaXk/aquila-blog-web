...
test.describe("admin posts workspace link contract", () => {
  ...
  test("V4 markdown editor는 code title snippet과 모바일 split preview를 유지한다", () => {
    ...
    expect(controllerSource).toContain('isRecommendTagsDisabled={disabled("recommendTags") || !postContent.trim()}')
    expect(controllerSource).toContain('isRecommendTagsLoading={loadingKey === "recommendTags"}')
    expect(controllerSource).toContain("onRecommendTags={() => void handleRecommendTags()}")
    ...
    expect(guideSource).toContain("<EditorInspectorFullWidthAction")
    expect(guideSource).toContain("onClick={onRecommendTags}")
    expect(guideSource).toContain('disabled={isRecommendTagsDisabled || isRecommendTagsLoading}')
    expect(guideSource).toContain('isRecommendTagsLoading ? "추천 중..." : "AI 태그 추천"')
