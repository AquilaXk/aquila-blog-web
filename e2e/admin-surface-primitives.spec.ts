...
test.describe("관리자 표면 공통 계약", () => {
  ...
  test("관리자 사이드바는 V4 reference rail 순서와 하단 프로필을 유지한다", () => {
    ...
    expect(source).toContain("profileSnapshot?: AdminProfile | null")
    expect(source).toContain('const sidebarIdentityName = (member.nickname || member.username || "관리자").trim()')
    expect(source).toContain("useAdminShellProfile(member.id, profileSnapshot)")
    expect(source).toContain('const brandTitle = profile?.blogTitle?.trim() || ""')
    expect(source).toContain("const sidebarProfileImageSrc = profile?.profileImageUrl.trim() || \"\"")
    expect(source).toContain("setIsProfileImageFailed(false)")
    expect(source).toContain('"프로필 정보를 불러올 수 없습니다."')
    expect(source).not.toContain("member.profileImage")
    expect(source).not.toContain("CONFIG.profile.image")
    expect(source).not.toContain("CONFIG.blog.title")
