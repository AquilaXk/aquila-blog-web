...
test.describe("관리자 표면 공통 계약", () => {
  test("관리자 표면은 MYBOX형 화이트 우선 시스템 테마 토큰을 공유한다", () => {
    ...
    expect(colorTokenSource).toContain("--admin-primary: #0a58ca;")
    ...
    expect(colorTokenSource).toContain('export const adminSurfaceAccent = "var(--admin-surface-accent, #eef6ff)"')
    expect(colorTokenSource).toContain('export const adminAccentText = "var(--admin-accent-text, #0969da)"')
    ...
    expect(shellSource).toContain("grid-template-columns: 14.75rem minmax(0, 1fr);")
    ...
    expect(utilitySource).toContain('searchMode?: "shortcuts" | "hidden"')
    expect(utilitySource).toContain('<SearchSpacer aria-hidden="true" />')
    expect(utilitySource).toContain("@media (max-width: 720px)")
    expect(utilitySource).toContain("display: none;")
  ...
  test("관리자 MYBOX 색상 토큰은 brown gold 계열 대신 neutral blue accent를 사용한다", () => {
    ...
    expect(tokenSource).toContain("--admin-primary: #0969da;")
    expect(tokenSource).toContain("--admin-primary-hover: #075bb5;")
    ...
    expect(getContrastRatio("#f8fafc", "#0a58ca")).toBeGreaterThanOrEqual(4.5)
