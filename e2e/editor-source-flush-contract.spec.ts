...
test.describe("editor save flush source contract", () => {
  ...
  test("저장/발행/브라우저 임시저장은 pending editor markdown을 먼저 flush한다", () => {
    ...
    expect(blockEditorMarkdownCommitSource).toContain("flushPendingNodeViewAttributeCommits(pendingEditor.view.dom)")
    expect(nodeViewCommitRegistrySource).toContain("export type NodeViewCommitFlusher = () => void")
    expect(nodeViewCommitRegistrySource).toContain("scopedPendingNodeViewCommitFlushers")
    expect(nodeViewCommitRegistrySource).toContain("export const flushPendingNodeViewAttributeCommits = (scope?: object | null)")
    expect(editorNodeViewSharedSource).toContain("registerPendingNodeViewCommitFlusher(flush, flushScope)")
    expect(mermaidNodeViewModelSource).toContain('export { useDebouncedAttributeCommit } from "./editorNodeViewShared"')
    expect(mermaidNodeViewModelSource).not.toContain("registerPendingNodeViewCommitFlusher")
