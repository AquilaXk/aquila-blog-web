...
test("auth probe 전송 실패는 헤더에 기술 오류를 노출하거나 Login 진입을 막지 않는다", async ({
  page,
}) => {
  await mockFeedEndpoints(page)
  await page.setViewportSize({ width: 1800, height: 900 })
  ...
  await page.goto("/", { waitUntil: "domcontentloaded" })
  await expect.poll(() => authProbeFailed).toBe(true)
  await expect(page.locator(".authArea")).toHaveAttribute("data-auth-state", "unavailable")
  await expect(page.getByText("Auth check failed")).toHaveCount(0)
