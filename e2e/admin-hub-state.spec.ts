...
test.describe("admin hub state contract", () => {
  test("관리자 허브는 protected bootstrap profile만 first paint seed로 사용한다", () => {
    ...
    expect(source).toContain("const baseResult = await timed(() => getAdminPageProps(req))")
    expect(source).not.toContain("resolvePublicAdminProfileSnapshot")
    expect(source).not.toContain("fetchServerAdminProfile")
    expect(source).not.toContain("adminProfileResultPromise")
    expect(source).not.toContain("publicProfilePromises")
    expect(source).toContain('profileDescription = "unavailable"')
    ...
    expect(source).toContain("const displayName = sessionMember?.nickname || sessionMember?.username || \"관리자\"")
    expect(source).toContain('"프로필 정보를 불러올 수 없습니다."')
    expect(source).not.toContain("sessionMember?.profileRole")
    expect(source).not.toContain("sessionMember?.profileImage")
