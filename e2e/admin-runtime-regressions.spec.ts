...
test.describe("관리자 런타임 회귀 계약", () => {
  ...
  test("새로고침 전 첫 페인트는 저장된 테마나 시스템 다크 모드를 먼저 적용한다", () => {
    ...
    expect(documentSource).not.toContain('var darkPreHydrationGuard = nextScheme === "dark"')
    expect(documentSource).not.toContain('html[data-aquila-scheme-bootstrap="dark"] *{color-scheme:dark!important;}')
    expect(documentSource).not.toContain('background-color:transparent!important;background-image:none!important;box-shadow:none!important;')
    expect(documentSource).not.toContain('[role="search"],[class*="Card"],[class*="card"]')
    expect(documentSource).not.toContain('" + darkPreHydrationGuard')
    expect(documentSource).not.toContain("#111318")
    expect(documentSource).toContain('html[data-aquila-scheme-bootstrap]{background:" + background + ";color-scheme:" + nextScheme + ";}')
    expect(documentSource).toContain("html[data-aquila-scheme-bootstrap] body")
    expect(schemeSource).toContain("const resolveBootstrapScheme = (): SchemeType | null =>")
    expect(schemeSource).toContain('import { useCallback, useEffect, useLayoutEffect } from "react"')
    expect(schemeSource).toContain('const useIsomorphicLayoutEffect = typeof window === "undefined" ? useEffect : useLayoutEffect')
    ...
    expect(schemeSource).toContain("resolveBootstrapScheme() ??")
    expect(schemeSource).not.toContain("getCookie")
