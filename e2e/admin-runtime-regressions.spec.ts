...
test.describe("관리자 런타임 회귀 계약", () => {
  test("운영 브라우저 API는 로그인과 관리자 요청, 알림 snapshot, 작성 temp draft를 same-origin 백엔드 프록시로 보낸다", () => {
    ...
    expect(proxySource).toContain("new Promise<ProxyBodyReadResult>")
    ...
    expect(proxySource).toContain("const DEFAULT_BACKEND_PROXY_MAX_BODY_BYTES = 104 * 1024 * 1024")
    expect(proxySource).toContain("const DEFAULT_BACKEND_PROXY_MAX_IN_FLIGHT_BODY_BYTES = 256 * 1024 * 1024")
    ...
    expect(proxySource).toContain("body: toArrayBuffer(Buffer.concat(chunks))")
