...
test.describe("관리자 런타임 회귀 계약", () => {
  test("운영 브라우저 API는 로그인과 관리자 요청을 same-origin 백엔드 프록시로 보낸다", () => {
    ...
    expect(proxySource).toContain("const resolveProxyBody = (req: NextApiRequest, method: string): BodyInit | undefined =>")
    expect(proxySource).toContain("Readable.toWeb(req) as unknown as BodyInit")
