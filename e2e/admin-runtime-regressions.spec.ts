...
test.describe("관리자 런타임 회귀 계약", () => {
  ...
  test("새로고침 전 첫 페인트는 light-only bootstrap을 먼저 적용한다", () => {
    ...
    expect(documentSource).not.toContain("AQUILA_CONFIGURED_SCHEME")
    expect(documentSource).not.toContain("prefers-color-scheme: dark")
    expect(documentSource).not.toContain("darkPreHydrationGuard")
    expect(documentSource).toContain('var nextScheme = "light"')
    expect(documentSource).toContain('var bootstrapSource = "configured"')
    expect(documentSource).toContain('var background = "#f7f7f5"')
    expect(documentSource).toContain('document.documentElement.dataset.aquilaScheme = nextScheme')
    expect(documentSource).toContain('document.documentElement.setAttribute("data-aquila-scheme-config", "light")')
