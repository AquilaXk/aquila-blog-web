...
test.describe("core smoke detail layout", () => {
  test("상세 shell은 V4 article layout 수치 계약을 유지한다", () => {
    ...
    expect(detailStyles).toContain("border-bottom: 1px solid var(--aq-border);")
    ...
    expect(detailStyles).toContain("border-left: 1px solid var(--aq-border);")
    ...
    expect(detailStyles).toContain("@media (max-width: ${layoutBreakpoint.adminCompact}px)")
    expect(detailStyles).toContain("grid-template-columns: 52px minmax(0, 1fr);")
    expect(detailStyles).toContain('.detailLayout[data-toc="true"]')
    expect(detailStyles).toContain("grid-template-columns: 52px minmax(0, 1fr) minmax(148px, 180px);")
    expect(detailStyles).toContain('.detailLayout[data-toc="false"] .rightRail')
    expect(detailSource).toContain('data-toc={showStickyToc ? "true" : "false"}')
    expect(detailStyles).not.toContain("width: min(100%, 864px);")
    expect(detailStyles).toContain("@media (max-width: ${layoutBreakpoint.navCompact}px)")
